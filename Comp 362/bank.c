...
bool isSafeState(int customerNum, double *request)
{
    ...
    //Allocate and create a copy of the bank and the customers
    //Pretend to grant the loan to the customer
    //Run the safety algorithm
    //Return the result of the safety test
    if(){
...
{
    bool ret = lessOrSame(request, need[customerNum], numOfAccounts);
    bool rex = lessOrSame(request, available, numOfAccounts);
    if(ret && rex){
	return isSafeState(customerNum, request);	
    } 
    return false; 
}
...
bool borrow(int customerNum, double funds[])
{
   ...
   if(ret){
	...
	sub(available, funds, numOfAccounts);
  	sub2(need, maximum[numOfAccounts], allocation[customerNum], numOfAccounts);	   
   }
...
bool repay(int customerNum, double funds[])
{
    ...
    //Checks whether the customer has enough money allocated (borrowed) to repay the specified amount.
    //If the above check passes then proceed with the repayment request.
    //Return a boolean indicating whether or not the payment succeeded
    if(ret){
